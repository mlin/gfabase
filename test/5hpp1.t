...
bgzip -@ 4 -dc /tmp/GRCh38-freeze1.gfa.gz | time $gfabase load - "${TMPDIR}/GRCh38-freeze1.gfab" --compress 1
is "$?" "0" "gfabase load"
...
$gfabase sub "${TMPDIR}/GRCh38-freeze1.gfab" "${TMPDIR}/CR1.gfab" --cutpoints 1 215577
is "$(gsql "${TMPDIR}/CR1.gfab" 'select count(1) from gfa1_segment_meta')" "16" "CR1 --cutpoints 3 segment count"
is "$(gsql "${TMPDIR}/CR1.gfab" 'select count(1) from gfa1_link')" "22" "CR1 --cutpoints 3 link count"
$gfabase sub "${TMPDIR}/GRCh38-freeze1.gfab" "${TMPDIR}/CR1.3.gfab" --cutpoints 3 215577
is "$(gsql "${TMPDIR}/CR1.3.gfab" 'select count(1) from gfa1_segment_meta')" "29" "CR1 --cutpoints 3 segment count"
is "$(gsql "${TMPDIR}/CR1.3.gfab" 'select count(1) from gfa1_link')" "41" "CR1 --cutpoints 3 link count"

$gfabase sub "${TMPDIR}/GRCh38-freeze1.gfab" "${TMPDIR}/RHD.gfab" --cutpoints 1 --range chr1:25,272,509-25,330,445
is "$(gsql "${TMPDIR}/RHD.gfab" 'select count(1) from gfa1_segment_meta')" "17" "RHD --cutpoints 3 segment count"
