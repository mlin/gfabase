...
-- of a desired subgraph. From attached input.{{prefix}}* it copies the segments and the links
...
INSERT INTO temp.sub_paths(path_id)
    SELECT path_id FROM input.{{prefix}}gfa1_path
    WHERE path_id NOT IN
        -- paths with missing segments:
        (SELECT DISTINCT path_id FROM input.{{prefix}}gfa1_path_element
         WHERE segment_id NOT IN temp.sub_segments);

INSERT INTO {{prefix}}gfa1_path(path_id, name, tags_json)
    SELECT path_id, name, tags_json FROM input.{{prefix}}gfa1_path
    WHERE path_id IN temp.sub_paths;

INSERT INTO {{prefix}}gfa1_path_element(path_id, ordinal, segment_id, reverse, cigar_vs_previous)
    SELECT path_id, ordinal, segment_id, reverse, cigar_vs_previous FROM input.{{prefix}}gfa1_path_element
    WHERE path_id IN temp.sub_paths;
