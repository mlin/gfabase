...
WITH RECURSIVE
    upstream(segment_id) AS (
        ...
        UNION
        SELECT from_segment FROM gfa1_link, upstream WHERE to_segment = upstream.segment_id
    )
...
WITH RECURSIVE
    connected(segment_id) AS (
        ...
        UNION
        SELECT to_segment FROM gfa1_link, connected WHERE from_segment = connected.segment_id
    )
